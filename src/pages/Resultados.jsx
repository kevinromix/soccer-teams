...
export default function Resultados(props) {
    ...
    const columns = Object.keys(results[0] ?? []).slice(1).map(k => {
        return {
            title: () => <div>{k}</div>,
            width: 75,
    ...
    const columnsAvg = Object.keys(results[0] ?? []).slice(1).map(k => {
        return {
            title: () => <div>{k}</div>,
            width: 75,
            fixed: k === 'Jornada' ? 'left' : '',
    ...
    const marcadoresColumn = [
        {
            title: <div><span>Marcadores</span></div>,
            render: (text, record) => {
                return (
                    <div>
        ...
        {
            title: () => <div><span>Repeticiones</span></div>,
            render: (text, record) => {
                return <div><span>{record.value}</span></div>;
            },
            sorter: (a, b) => {
                a = a.value;
                b = b.value;
...
async function getFirstGames(competitionId) {
    return await fetch(`https://webws.365scores.com/web/games/results/?appTypeId=5&langId=29&timezoneName=America/Mexico_City&userCountryId=31&competitions=${competitionId}&showOdds=true&includeTopBettingOpportunity=1&topBookmaker=14`)
        .then(async response => await response.json());
}

async function getNextGames(competitionId, lastGameId) {
    return await fetch(`https://webws.365scores.com/web/games/?langId=29&timezoneId=78&userCountryId=31&apptype=5&competitions=${competitionId}&games=1&aftergame=${lastGameId}&direction=-1&withmainodds=true`)
        .then(async response => await response.json());
}

async function setNextGames(competitionId, results, marcadores, seasonNum, lastGameId) {
...
async function fetchData(competitionId, results, marcadores) {
    let lastGameId;
    let seasonNum;
    await getFirstGames(competitionId).then((response) => {
